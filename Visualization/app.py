...
def instagram():
    ...
    projects = collection.find({},{'caption': 1,'commentCount': 1, 'likesCount': 1, 'created_time': 1, 'url': 1, '_id': 1, 'latitude': 1, 'longitude': 1})
    json_projects = []
    for project in projects:
        json_projects.append(project)
    json_projects = json.dumps(json_projects, default=json_util.default)
...
def twitter():
    ...
    projects = collection.find({},{'name': 1,'created_at': 1, 'as_of': 1, 'url': 1, '_id': 0})
    json_projects = []
    for project in projects:
        json_projects.append(project)
    json_projects = json.dumps(json_projects, default=json_util.default)
